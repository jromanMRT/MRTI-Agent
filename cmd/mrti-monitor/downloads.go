...
document.getElementById('brandLogo').src=portalOrigin+'/company-logo.svg';fetch(portalOrigin+'/api/portal/v1/brand-appearance',{cache:'no-store'}).then(response=>response.ok?response.json():Promise.reject()).then(({data})=>{if(data?.portal_logo?.content_url)document.getElementById('brandLogo').src=portalOrigin+data.portal_logo.content_url}).catch(()=>{});
